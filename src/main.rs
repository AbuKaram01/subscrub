...
use subscrub::core::{
    downloader::{
        ...
        is_valid_youtube_url, list_available_subs, require_ffmpeg, require_yt_dlp,
...
use cli::ui::{
    ask_dir, ask_file, ask_format, ask_languages, ask_merge_type, ask_output_dir, ask_sub_files,
    ask_sub_type, ask_task, ask_url, make_spinner, print_banner, show_browser,
};
use cli::{
    detect_download_mode, detect_merge_mode, parse_format, parse_languages, parse_sub_type, Cli,
    MergeType, Mode, Task,
};
...
fn run_playlist(
    ...
    println!(
        ...
        style(languages.len()).cyan().bold(),
        if languages.len() == 1 { "" } else { "s" },
        style("(based on first video)").dim()
    );
        ...
        match result {
            Ok(_) => {
                ...
                println!(
                    ...
                    style("✓").green().bold(),
                    stage_label,
                    style(format!("{}.mkv", job.output_name)).dim()
                );
            }
            Err(e) => eprintln!(
                "  {}  {}  {}",
...
fn main() {
    ...
    match task {
        ...
        Task::Download => {
            ...
            if url.is_empty() {
                eprintln!("  {}  No URL provided.", style("✗").red().bold());
                std::process::exit(1);
            }
